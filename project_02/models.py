...
class SmoothClassifier(nn.Module):
    ...
    def certify(self, inputs: torch.Tensor, n0: int, num_samples: int, 
        ...
        num_class_A=self._sample_noise_predictions(inputs,num_samples,batch_size)[top_class]
    ...
    def predict(self, inputs: torch.tensor, num_samples: int, alpha: float, 
        ...
        ##########################################################
        # YOUR CODE HERE
        class_ord=torch.argsort(class_counts,descending=True)
        first_index=class_ord[0]
        count1=class_counts[first_index]
        second_index=class_ord[1]
        count1=class_counts[second_index]
    ...
    def _sample_noise_predictions(self, inputs: torch.tensor, num_samples: int, 
        ...
        with torch.no_grad():
            ...
            for it in range(ceil(num_samples / batch_size)):
                ...
                ##########################################################
                # YOUR CODE HERE
                this_batch = inputs.repeat(this_batch_size,1,1,1)
                logits = self.forward(x) #(B,K)
                pred_class = torch.argmax(logits,dim=1) #(B,) each int in [0,K-1]
                for i in pred_class:
                    class_counts[i] += 1
