...
def train_model(model: nn.Module, dataset: Dataset, batch_size: int, 
    ...
    for epoch in range(epochs):
        ...
        for x,y in tqdm(train_loader, total=num_train_batches):
            ...
            #in mnist label(y) 0-9 image 0-255 28*28
            loss,logits=loss_function(x, y, model, **loss_args) #logits: one prob for each class 0-9 size:batch_size*10
            ...
            output_label=torch.argmax(logits,dim=1)
            accuracy=torch.sum(output_label == y)/batch_size
...
def predict_model(model: nn.Module, dataset: Dataset, batch_size: int, 
    ...
    for x, y in tqdm(test_loader, total=num_batches):
        ...
        if attack_function is None:
            ...
            per_x=attack_function(logits, x, y, **attack_args)
            logits=model(per_x).cpu()
        output_label=torch.argmax(logits,dim=1)    
...
def evaluate_robustness_smoothing(base_classifier: nn.Module, 
    ...
    for x, y in tqdm(test_loader, total=len(dataset)):
        ##########################################################
        # YOUR CODE HERE
        top_class, radius=model.certify(inputs=x, n0=num_samples_1, num_samples=num_samples_2, alpha=alpha, batch_size=certification_batch_size)
        if top_class==-1:
            abstains+=1
        elif top_class==y:
            correct_certified+=1
